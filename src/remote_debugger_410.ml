...
let attach conn =
  ( module struct
    ...
    let initial_frame () =
      ...
      let%lwt pos = Lwt_io.BE.read_int conn.in_ in
      Log.debug (fun m ->
          m "initial_frame return %s"
            ([%show: int * pc] (stack_pos, { frag; pos })));%lwt
    ...
    let up_frame stacksize =
      ...
      in
      Log.debug (fun m ->
          m "set_frame return %s" ([%show: (int * pc) option] res));%lwt
      Lwt.return res
    ...
    let get_closure_code rv =
      ...
      let%lwt pos = Lwt_io.BE.read_int conn.in_ in
      Log.debug (fun m ->
          m "get_closure_code return %s" (show_pc { frag; pos }));%lwt
      Lwt.return { frag; pos }

    let set_fork_mode mode =
